...
// Define a type for the replacement mapping
...
const pathsToInclude: string[] = ['packages/gestalt', '*.css'];
const pathsToExclude: string[] = ['gestalt.css', 'node_modules', 'dist'];
...
for (const termToFind of Object.keys(replaceMap)) {
  const termToReplace: string = replaceMap[termToFind];
  console.log(' ');
  console.log('Replacing all instances of', termToFind, 'with', termToReplace);
  console.log(' ');
  const command: string = `replace \\"${termToFind}\\" \\"${termToReplace}\\"  .. -r --exclude="${pathsToExclude.join(
    ',',
  )}" --include="${pathsToInclude.join(',')}"`;
  const resp: Buffer = execSync(command);
